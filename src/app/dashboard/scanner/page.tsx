      ...
      <div className="grid grid-cols-1 gap-8">
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-6">
            ...
            {/* ── CV Selection Widget & Manual Input Trigger ────────────────── */}
              ...
              <div className="flex items-center justify-between">
                ...
                {savedCvs.length > 0 && (
                  <span className="text-xs text-muted-foreground font-normal">
                    {savedCvs.length} {savedCvs.length === 1 ? "CV" : "CVs"} in CV Manager
                  </span>
                )}
              </div>

              {/* Compact CV Manager Trigger Card */}
              {isSavedCvsLoading ? (
                <div className="p-4 border border-zinc-200 rounded-xl animate-pulse flex items-center justify-center text-sm text-muted-foreground">
                  Loading saved CVs...
                </div>
              ) : (
                <div className="p-4 border-2 border-zinc-200 bg-white rounded-xl shadow-sm hover:border-zinc-300 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="p-3 bg-[#FFF3EB] text-[#FF6B00] rounded-xl flex-shrink-0">
                        ...
                        <>
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-bold text-[#1D1D1F] truncate">
                              {savedCvs.find((c) => c.id === selectedCvId)?.file_name || "Selected Saved CV"}
                            </p>
                            <Badge className="bg-[#FF6B00] text-white text-[10px] px-2 py-0.5">
                              Active
                            </Badge>
                          </div>
                          <p className="text-xs text-zinc-500 mt-0.5">
                            Loaded from CV Manager · Ready for match analysis
                          </p>
                        </>
                      ) : (
                        <>
                          <p className="text-sm font-semibold text-[#1D1D1F]">
                            {savedCvs.length > 0 ? "Choose a Saved CV" : "No Saved CVs Found"}
                          </p>
                          <p className="text-xs text-zinc-500 mt-0.5">
                            {savedCvs.length > 0
                              ? "Select from your saved CVs or upload a new one below"
                              : "Upload your first CV below to save it to your CV Manager"}
                          </p>
                        </>
                      )}
                    </div>
                  ...
                  {savedCvs.length > 0 && (
                    <Button
                      ...
                      className="bg-[#FF6B00] hover:bg-[#E55F00] text-white font-medium text-xs px-4 py-2 rounded-lg flex items-center gap-2 flex-shrink-0 w-full sm:w-auto justify-center"
                    >
                      <FolderOpen className="h-4 w-4" />
                      {selectedCvId ? "Change CV" : `Choose from Saved (${savedCvs.length})`}
                    </Button>
                  )}
                </div>
              )}
            ...
            {/* ── Saved CV Selection Modal Widget ───────────────────────────── */}
            <Dialog open={isCvModalOpen} onOpenChange={setIsCvModalOpen}>
              <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col p-6 bg-white rounded-2xl">
                <DialogHeader className="space-y-1">
                  <DialogTitle className="text-xl font-bold text-[#1D1D1F] flex items-center gap-2">
                    <FolderOpen className="h-5 w-5 text-[#FF6B00]" />
                    Select a Saved CV
                  </DialogTitle>
                  <DialogDescription className="text-xs text-zinc-500">
                    Choose from your {savedCvs.length} saved CVs or preview their contents to find the right version.
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="relative my-2">
                  ...
                  <Input
                    placeholder="Search by CV filename..."
                    value={cvSearchQuery}
                    onChange={(e) => setCvSearchQuery(e.target.value)}
                    className="pl-9 bg-zinc-50 border-zinc-200 text-sm focus:bg-white"
                  />
                </div>

                {/* Filtered CV List */}
                <div className="flex-1 overflow-y-auto space-y-3 pr-1 max-h-[420px] my-2">
                  {savedCvs
                    .filter((cv) =>
                      cv.file_name.toLowerCase().includes(cvSearchQuery.toLowerCase())
                    )
                    .map((cv) => {
                      const isSelected = selectedCvId === cv.id;
                      const isPreviewing = previewingCvId === cv.id;
                      ...
                      return (
                        <div
                          key={cv.id}
                          className={`rounded-xl p-4 border-2 transition-all flex flex-col gap-3 ${
                            isSelected
                              ? "border-[#FF6B00] bg-[#FFF3EB]"
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                              <div
                                className={`p-2.5 rounded-lg ${
                                  isSelected ? "bg-[#FF6B00] text-white" : "bg-zinc-100 text-zinc-600"
                                }`}
                              >
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-zinc-500 mt-0.5">
                                  ...
                                  <span className="font-semibold text-zinc-700">{ext}</span>
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setPreviewingCvId(isPreviewing ? null : cv.id)
                                }
                                className="text-xs flex items-center gap-1.5 border-zinc-300 hover:bg-zinc-100"
                              >
                                {isPreviewing ? (
                                  <>
                          ...
                          {isPreviewing && (
                            <div className="mt-1 p-3 bg-zinc-900 text-zinc-100 rounded-lg text-xs leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto font-mono border border-zinc-800">
                              <p className="text-[10px] text-zinc-400 font-sans uppercase tracking-wider mb-1 font-bold border-b border-zinc-800 pb-1">
                                CV Preview Snippet
                              </p>
                              {cv.file_content || "No text content available in this CV."}
                            </div>
                          )}
                        </div>
                      );
                  ...
                  {savedCvs.filter((cv) =>
                    ...
                    <div className="p-6 text-center text-sm text-zinc-500 bg-zinc-50 rounded-xl">
                      No CVs match &quot;{cvSearchQuery}&quot;.
                    </div>
                  )}
                </div>
              </DialogContent>
