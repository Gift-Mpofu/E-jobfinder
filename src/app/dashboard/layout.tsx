...
import { Bell, User, LogOut, CheckCircle, BrainCircuit, Timer } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // State for usage and notifications
  ...
  return (
    <DashboardContext.Provider value={contextValue}>
      <div className="min-h-screen bg-background text-foreground">
        <header className="p-4 border-b border-border/40 sticky top-0 bg-background/80 backdrop-blur-sm z-50">
          <div className="container mx-auto flex items-center justify-between">
            <Link href="/dashboard" className="text-2xl font-bold text-primary font-headline">Angine</Link>
              ...
              <Popover open={notificationsOpen} onOpenChange={setNotificationsOpen}>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="icon">
                    ...
                    {notifications.length > 0 && <span className="absolute top-0 right-0 h-2 w-2 rounded-full bg-primary" />}
                  </Button>
                </PopoverTrigger>
